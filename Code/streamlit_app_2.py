...
def get_cosine_similarity_score(text1, text2):
    corpus = [text1, text2]
    corpus = [str(asdfasddsadsa) for asdfasddsadsa in corpus]
...
# # make df into list
job_ad_text_2 = df['job_ad_text']
# merged_texts = [str(asdfasddsadsa) for asdfasddsadsa in merged_texts]


# Calculate the similarity score and buzzword count
if job_ad_text:
    buzzwords = get_both()
    similarity_score = get_cosine_similarity_score(job_ad_text_2, ' '.join(buzzwords))
